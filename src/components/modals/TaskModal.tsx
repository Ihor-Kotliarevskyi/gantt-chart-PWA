...
export const TaskModal: React.FC<TaskModalProps> = ({ isOpen, onClose, taskId }) => {
  ...
  const handleSave = () => {
    ...
    if (ms * 4 + ws > me * 4 + we) return alert('Початок не може бути після кінця');
    
    const taskData = { name, cat, contr, ms, ws, me, we, prog, budget, spent, deps };
  ...
  const handleDepToggle = (n: number) => {
    setDeps(prev => prev.includes(n) ? prev.filter(x => x !== n) : [...prev, n]);
  };
  ...
  return (
    <Modal title={taskId === null ? "Нова робота" : "Редагувати роботу"} isOpen={isOpen} onClose={onClose}>
      <div className="fg">
        <label>Назва</label>
        <input value={name} onChange={e => setName(e.target.value)} placeholder="Введіть назву..." />
      </div>
      <div className="fg">
      ...
      <div className="fg">
        ...
        <input value={contr} onChange={e => setContr(e.target.value)} placeholder="необов'язково" />
      </div>
      ...
      <div className="row2">
        <div className="fg">
          ...
          <select value={ms} onChange={e => setMs(+e.target.value)}>
            {ml.map(m => <option key={m.i} value={m.i}>{m.i + 1}. {m.name} {m.y}</option>)}
          </select>
        </div>
        <div className="fg">
          <label>Тиждень</label>
      ...
      <div className="fg">
        ...
        <div className="prog-row" style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          <input type="range" min="0" max="100" step="5" value={prog} onChange={e => setProg(+e.target.value)} style={{ flex: 1 }} />
          <span>{prog}%</span>
        </div>
      </div>
      ...
      <div className="row2">
        <div className="fg">
          ...
          <input type="number" min="0" step="1000" value={spent || ''} onChange={e => setSpent(+e.target.value)} />
        </div>
        <div className="fg">
          <label>Залишок</label>
          <input readOnly value={restText} style={{ background: 'var(--surf2)', color: 'var(--txt2)' }} />
        </div>
      </div>
      ...
      <div className="fg">
        ...
        <div className="dep-list">
          {tasks.filter(t => t.n !== taskId).map(t => (
            <label key={t.n} className="dep-item" style={{ display: 'flex', gap: '5px', alignItems: 'center' }}>
              <input 
                type="checkbox" 
