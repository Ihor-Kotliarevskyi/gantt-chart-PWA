...
export const ChartsTab: React.FC = () => {
  ...
  const autoCharts = [
    ...
    { id: 'a3', type: 'doughnut', x: 'status', y: 'count', title: 'Статус робіт (кількість)' },
    { id: 'a4', type: 'bar', x: 'task', y: 'dur', title: 'Тривалість робіт (тиж.)' },
    { id: 'a5', type: 'line', x: 'month', y: 'count', title: 'Нових робіт за місяцями' },
  ];

  const renderChart = (c: any) => {
    const { labels, values } = getChartData(tasks, cats, proj, c.x, c.y, "", "", new Set());
    ...
    const options: any = {
        ...
        tooltip: {
          callbacks: {
            label: (context: any) => {
                ...
                if (label) label += ': ';
                if (context.parsed.y !== undefined) label += fmtM(context.parsed.y);
                else if (context.parsed !== undefined) label += fmtM(context.parsed);
                return label;
            }
          }
        }
      ...
      scales: (c.type === 'pie' || c.type === 'doughnut') ? {} : {
        ...
        y: { ticks: { font: { size: 9 }, callback: (v: any) => fmtM(v) } },
      },
    };

    return (
      <div key={c.id} className="chart-card">
        <h4><span>{c.title}</span></h4>
